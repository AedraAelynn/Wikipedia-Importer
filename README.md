...
| Setting | Options | Notes |
|---|---|---|
| **Link style** | Wikilinks / Markdown links / No links | *Wikilinks* (`[[Page]]`) build your `Vault` graph. *Markdown links* point to Wikipedia online and will not create a `Note` — ideal for a standalone reading copy. *No links* strips linking entirely. |
...
4. In Obsidian: **Settings → Community plugins**, enable `Community Plugins` if needed, then `Enable` **Wikipedia Importer**, and adjust settings to your liking.
